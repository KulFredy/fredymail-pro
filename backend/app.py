...
def meili_get_facets(attribute: str) -> dict[str, int]:
    payload = {"q": "", "limit": 0, "facets": [attribute]}
...
@app.post("/move_bulk")
async def move_emails_bulk(req: BulkMoveRequest):
    docs = []
    errors = []
    for eid in req.ids:
        try:
    ...
    if docs:
        result = meili_update_documents(docs)
        return {"status": "ok", "moved": len(docs), "errors": errors, "task": result}
    return {"status": "ok", "moved": 0, "errors": errors}
...
async def awaiting_reply():
    ...
    query_str = " ".join(sample_subjects[:5])
...
_AI_CHAT_SYSTEM = """Sen FredyMail Pro'nun akıllı e-posta asistanısın. Türkçe yanıt ver.

Kullanıcı mesajını analiz et ve şu JSON formatında yanıt ver:
{
  "intent": "search" | "move" | "summarize" | "answer",
  "reply": "Kullanıcıya gösterilecek kısa Türkçe yanıt",
  "search_query": "Meilisearch için arama sorgusu (sadece search/summarize için)",
  "search_filters": {
    "folder": "klasör adı veya null",
    "sender": "gönderen filtresi veya null",
    "year": 2026 veya null
  },
  "move_action": {
    "filter_sender": "gönderen email adresi veya null",
    "filter_folder": "kaynak klasör veya null",
    "target_folder": "hedef klasör"
  }
}

Örnekler:
- "aykut sipariş" → intent: search, search_query: "aykut sipariş"
- "aykut@zoomithalat.com adresinden gelenleri Aykut klasörüne taşı" → intent: move, move_action: {filter_sender: "aykut@zoomithalat.com", target_folder: "Aykut"}
- "Son fatura maillerini özetle" → intent: summarize, search_query: "fatura"
...
async def ai_chat(req: AIChatRequest):
    """Smart AI chat: understands search, move, summarize intents."""
    prompt = f"{_AI_CHAT_SYSTEM}\n\nKullanıcı mesajı: {req.message}"
    ...
    try:
        ...
        return {"intent": "answer", "reply": raw, "hits": []}

    intent = ai_resp.get("intent", "answer")
    reply = ai_resp.get("reply", "")
    hits: list[dict] = []

    if intent in ("search", "summarize"):
        sq = ai_resp.get("search_query", req.message)
        sf = ai_resp.get("search_filters", {}) or {}
        search_payload: dict[str, Any] = {
            "q": sq,
            "limit": 20,
            "matchingStrategy": "all",
            "attributesToRetrieve": ["id", "subject", "sender", "date", "timestamp", "folder", "body"],
        }
        filters = []
        if sf.get("folder"):
            safe = str(sf["folder"]).replace("'", "\\'")
            filters.append(f"folder = '{safe}'")
        if sf.get("sender"):
            safe = str(sf["sender"]).replace("'", "\\'")
            filters.append(f"sender = '{safe}'")
        if sf.get("year"):
            yr = int(sf["year"])
            filters.append(f"timestamp >= {int(datetime(yr,1,1).timestamp())} AND timestamp < {int(datetime(yr+1,1,1).timestamp())}")
        if filters:
            search_payload["filter"] = " AND ".join(f"({f})" for f in filters)
        try:
            data = meili_search(search_payload)
            hits = data.get("hits", [])
        except Exception:
            pass

        if intent == "summarize" and hits:
            bodies = "\n\n".join(
                f"Konu: {h.get('subject','')}\nKimden: {h.get('sender','')}\n{(h.get('body','') or '')[:500]}"
                for h in hits[:10]
            )
            sum_prompt = f"Bu {len(hits)} maili kısaca özetle (Türkçe, 3-5 madde):\n\n{bodies}"
            try:
                reply = gemini_generate(sum_prompt)
            except Exception:
                pass

    elif intent == "move":
        ma = ai_resp.get("move_action", {}) or {}
        target = ma.get("target_folder")
        if target:
            move_filters = []
            if ma.get("filter_sender"):
                safe = str(ma["filter_sender"]).replace("'", "\\'")
                move_filters.append(f"sender CONTAINS '{safe}'")
            if ma.get("filter_folder"):
                safe = str(ma["filter_folder"]).replace("'", "\\'")
                move_filters.append(f"folder = '{safe}'")
            search_payload = {
                "q": "",
                "limit": 1000,
                "attributesToRetrieve": ["id", "subject", "sender", "folder"],
            }
            if move_filters:
                search_payload["filter"] = " AND ".join(f"({f})" for f in move_filters)
            try:
                data = meili_search(search_payload)
                to_move = data.get("hits", [])
                if to_move:
                    for doc in to_move:
                        doc["folder"] = target
                    meili_update_documents(to_move)
                    reply = f"✅ {len(to_move)} mail '{target}' klasörüne taşındı."
                    hits = to_move[:10]
                else:
                    reply = "Taşınacak mail bulunamadı."
            except Exception as e:
                reply = f"Taşıma hatası: {e}"

    return {"intent": intent, "reply": reply, "hits": hits}
