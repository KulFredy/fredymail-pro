...
def fetch_folder(mail: imaplib.IMAP4_SSL, folder: str, since_date: str, state: dict) -> list[dict]:
    try:
    ...
    for uid_bytes in new_uids:
        ...
        try:
            ...
            docs.append({
                ...
                "date": msg.get("Date", ""),
                "timestamp": parse_timestamp(msg.get("Date", "")),
...
def poll_once(state: dict) -> None:
    since_date = (
        datetime.now().replace(hour=0, minute=0, second=0) - timedelta(days=FETCH_DAYS)
    ).strftime("%d-%b-%Y")
